# source code for MatchXML, which is based on the implementation of Amazon's XR-Transformer. Please comply with Amazon's license as follow: 
...
from cl_loss import DulConLoss
...
class TransformerMatcher(pecos.BaseClass):
    ...
    class TrainParams(pecos.BaseParams):  # type: ignore
        ...
        beta_one: float = 0.9
        beta_two: float = 0.98
        match_loss: bool = False
        tau: float = 0.05
        lambda_one: float = 1.0
        lambda_two: float = 1.0
        plus_ce_loss: bool = False
    ...
    def fine_tune_encoder(self, prob, val_prob=None, val_csr_codes=None):
        ...
        betas = (train_params.beta_one, train_params.beta_two)
        optimizer = AdamW(
            optimizer_grouped_parameters,
        ...
        temperature = train_params.tau
        criterion_dual = DulConLoss(temperature=temperature)
        ...
        for epoch in range(1, int(train_params.num_train_epochs) + 1):
            ...
            for batch_cnt, batch in enumerate(train_dataloader):
                ...
                if train_params.match_loss :
                    # cl loss of instance-label pair
                    features_ins = torch.unsqueeze(outputs["pooled_output"], 1)
                    features_ins = F.normalize(features_ins, dim=-1)
                    features_label = text_model_W_seq.to(self.device)
                    features_label = F.normalize(features_label, dim=-1)
                    labels = inputs["label_values"].to(self.device)
                    cl_loss_1 = criterion_dual(features_ins, features_label, labels)

                    # cl loss of label-instance pair
                    indices = inputs["instance_number"].cpu().detach().numpy()
                    labels = prob.Y[indices, :].todense()
                    indices = np.nonzero(labels)[1]
                    indices = list(set(indices.tolist()))
                    indices.sort()
                    labels = labels[:, indices]
                    indices = torch.tensor(indices, dtype=torch.long).view(1, -1).to(self.device)
                    text_model_W_seq, text_model_b_seq = self.text_model(
                    ...
                    features_label = torch.squeeze(features_label, 0)
                    features_ins = torch.squeeze(features_ins, 1)
                    cl_loss_2 = criterion_dual(features_label, features_ins, labels)                 

                    # sum of losses
                    c1, c2 = train_params.lambda_one, train_params.lambda_two
                    if train_params.plus_ce_loss:
                        loss += cl_loss_1 * c1 + cl_loss_2 * c2
                    else:
                        loss =  cl_loss_1 * c1 + cl_loss_2 * c2
                else:
                    loss = loss
        ...
        **kwargs,
        ...
        if not prob.is_tokenized:
            if saved_trn_pt and os.path.isfile(saved_trn_pt):
                ...
                # free the memory
                trn_tensors_two = trn_tensors
                trn_tensors = copy.deepcopy(trn_tensors_two.data)
                del trn_tensors_two
                import ctypes
        ...
        if val_prob is not None and not val_prob.is_tokenized:
            ...
            if saved_val_pt and os.path.isfile(saved_val_pt):
                ...
                # free the memory
                val_tensors_two = val_tensors
                val_tensors = copy.deepcopy(val_tensors_two.data)
                del val_tensors_two
                import ctypes
                libc = ctypes.CDLL("libc.so.6")
                libc.malloc_trim(0)               
