...
class XTransformer(pecos.BaseClass):
    ...
    class TrainParams(pecos.BaseParams):
        ...
        is_label2vec: bool = True
        label2vec_dir: str = None
    ...
    def train(
        ...
        **kwargs,
        ...
        if not train_params.do_fine_tune:
            ...
            if clustering is None:
                ...
                if label_feat is None:
                    ...
                    is_label2vec = train_params.is_label2vec
                    if is_label2vec:
                        file_path = train_params.label2vec_dir
                        with open(file_path, 'rb') as f:
                            label_emb = np.load(f)
                        label_feat = sk_normalize(label_emb)
        ...
        if not train_params.only_encoder:
            ...
            X_inst = sk_normalize(inst_embeddings)
            del inst_embeddings
            ...
            if train_params.fix_clustering:
                ...
                is_label2vec = train_params.is_label2vec
                if is_label2vec:
                    label_feat = LabelEmbeddingFactory.pifa(prob.Y, X_inst)
                    file_path = train_params.label2vec_dir
                    with open(file_path, 'rb') as f:
                        label_emb = np.load(f)
                    # label_feat = sk_normalize(label_feat)
                    label_feat = TransformerMatcher.concat_features(
                        label_feat,
                        label_emb,
                        normalize_emb=True,
                    )
                    ...
                    label_feat = LabelEmbeddingFactory.pifa(prob.Y, X_concat)
                clustering = Indexer.gen(
                    label_feat,
    ...
    def predict(
        ...
        **kwargs,
        ...
        if saved_pt and os.path.isfile(saved_pt):
            ...
            # free the memory
            text_tensors_two = text_tensors
            text_tensors = deepcopy(text_tensors_two.data)
            del text_tensors_two
            import ctypes
